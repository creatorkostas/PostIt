...
// HammerProgress is a lightweight snapshot sent during streaming hammer runs.
type HammerProgress struct {
	Done          bool           `json:"done"`
...
// buildSnapshot creates a HammerProgress from current HammerResults.
func buildSnapshot(r *HammerResults, elapsed time.Duration, done bool) *HammerProgress {
	p := &HammerProgress{
		Done:          done,
	...
	if p.TotalRequests > 0 {
		totalLat := atomic.LoadInt64(&r.totalLatency)
		p.AvgLatencyMs = float64(totalLat) / float64(p.TotalRequests) / float64(time.Millisecond)
		p.RPS = float64(p.TotalRequests) / elapsed.Seconds()
	}

	return p
}

// HammerStream runs the same load test as Hammer but sends progress snapshots
// to progressCh every 500ms. It closes the channel when the test finishes.
func (c *Client) HammerStream(req *models.Request, workers int, duration time.Duration, progressCh chan<- *HammerProgress) {
		...
		go func() {
			...
			for {
				...
				default:
					...
					case "GET":
						resp, err = r.Get(hitUrl)
					case "POST":
						resp, err = r.Post(hitUrl)
					case "PUT":
						resp, err = r.Put(hitUrl)
					case "DELETE":
						resp, err = r.Delete(hitUrl)
					case "PATCH":
						resp, err = r.Patch(hitUrl)
					default:
						resp, err = r.Get(hitUrl)
					}
					...
					// Reservoir Sampling
					count := atomic.LoadInt64(&results.TotalRequests)
	...
	// Channel to signal all workers finished
	...
	// Send progress every 500ms until workers finish
	for {
		select {
		case <-workersDone:
			...
			if results.TotalRequests > 0 {
				...
				if len(results.Latencies) > 0 {
					...
					if p95Idx >= len(results.Latencies) {
						p95Idx = len(results.Latencies) - 1
					}
					results.P95Latency = results.Latencies[p95Idx]

					p99Idx := int(float64(len(results.Latencies)) * 0.99)
					if p99Idx >= len(results.Latencies) {
						p99Idx = len(results.Latencies) - 1
					}
					results.P99Latency = results.Latencies[p99Idx]
				}
			}

			snapshot := buildSnapshot(results, actualDuration, true)
			snapshot.P95LatencyMs = results.P95Latency.Seconds() * 1000
			snapshot.P99LatencyMs = results.P99Latency.Seconds() * 1000
			progressCh <- snapshot
			return

		case <-ticker.C:
			elapsed := time.Since(startTime)
			progressCh <- buildSnapshot(results, elapsed, false)
		}
	}
}
