        ...
        let hammerRpsHistory = [];
        let hammerLatHistory = [];
        ...
        async function runHammer() {
            ...
            const streamEnabled = document.getElementById('hammer-stream').checked;
            ...
            try {
                if (streamEnabled) {
                    await runHammerStream(parseInt(workers), parseInt(duration));
                } else {
                    await runHammerBlocking(parseInt(workers), parseInt(duration));
                }
            } catch (e) { resultsDiv.textContent = "Error: " + e.message; }
        }

        // ── Streaming hammer (NDJSON chunks every 500ms) ──────────────────────
        async function runHammerStream(workers, duration) {
            const resp = await fetch('/api/hammer', {
                method: 'POST',
                body: JSON.stringify({ path: currentRequest.path, workers, duration, stream: true })
            });
        ...
        // ── Blocking hammer (original behavior) ───────────────────────────────
        async function runHammerBlocking(workers, duration) {
            const resp = await fetch('/api/hammer', {
                method: 'POST',
                body: JSON.stringify({ path: currentRequest.path, workers, duration })
            });
            const data = await resp.json();
            ...
            document.getElementById('hammer-results').innerHTML =
        ...
        // ── Shared UI updater for hammer progress ─────────────────────────────
        function updateHammerUI(progress) {
            document.getElementById('h-total').textContent = progress.totalRequests;
            document.getElementById('h-rps').textContent = progress.rps ? progress.rps.toFixed(1) : '0';
            document.getElementById('h-avg').textContent = progress.avgLatencyMs ? progress.avgLatencyMs.toFixed(1) + 'ms' : '0ms';
            // P99 not available during streaming – show real value when done
            document.getElementById('h-p99').textContent = progress.done
                ? (progress.p99LatencyMs ? progress.p99LatencyMs.toFixed(1) + 'ms' : '—')
                : '…';

            // Accumulate values so chart keeps history instead of resetting to zero
            hammerRpsHistory.push(progress.rps || 0);
            hammerLatHistory.push(progress.avgLatencyMs || 0);
            hammerChart.data.labels = hammerRpsHistory.map((_, i) => i + 1);
            hammerChart.data.datasets[0].data = hammerRpsHistory;
            hammerChart.data.datasets[1].data = hammerLatHistory;
            hammerChart.update();

            const sc = progress.statusCodes || {};
            const scStr = Object.keys(sc).length > 0
            ...
            let html =
                ...
                `Avg Latency: ${progress.avgLatencyMs ? progress.avgLatencyMs.toFixed(2) + 'ms' : '—'}\n` +
                `RPS: ${progress.rps ? progress.rps.toFixed(2) : '—'}\n` +
                `Elapsed: ${(progress.elapsedMs / 1000).toFixed(1)}s\n` +
                `Status Codes: ${scStr}\n`;

            if (progress.done) {
                // Full final results — same info you'd get without streaming
                html += `\nP95 Latency: ${progress.p95LatencyMs ? progress.p95LatencyMs.toFixed(2) + 'ms' : '—'}\n` +
                        `P99 Latency: ${progress.p99LatencyMs ? progress.p99LatencyMs.toFixed(2) + 'ms' : '—'}\n` +
                        `\n— DONE —`;
            }

            document.getElementById('hammer-results').innerHTML = html;
        }
...
note: req.note || ''
        ...
        function addUrlencodedRow() {
            const div = document.createElement('div'); div.className = 'header-row';
            div.innerHTML = `<input type="text" class="urlencoded-key" placeholder="Key"><input type="text" class="urlencoded-value" placeholder="Value"><button class="sidebar-action-btn" onclick="this.parentElement.remove()">✕</button>`;
            document.getElementById('req-urlencoded-list').appendChild(div);
        }

        function addHeaderRow() {
            const div = document.createElement('div'); div.className = 'header-row';
            div.innerHTML = `<input type="text" class="header-key" placeholder="Header Name"><input type="text" class="header-value" placeholder="Header Value"><button class="sidebar-action-btn" onclick="this.parentElement.remove()">✕</button>`;
            document.getElementById('req-headers-list').appendChild(div);
        }
